...
export class MathLiveSettingTab extends PluginSettingTab {
  ...
  private displayMathSymbolMode(containerEl: HTMLElement): void {
    ...
    desc.createEl('br');
    desc.createEl('br');
    desc.appendText('Each shortcut has a trigger, written one of three ways:');
    ...
    ul.createEl('li').append(
      'A hold, e.g. ',
      createEl('code', { text: 'c+d' }),
      ' — two plain keys where the first acts as a normal key and the second as its ' +
        'modifier: fires when the first key (',
      createEl('code', { text: 'c' }),
      ') is released while the second (',
      createEl('code', { text: 'd' }),
      ') is still held down. Press order doesn’t matter, only what’s held at ' +
